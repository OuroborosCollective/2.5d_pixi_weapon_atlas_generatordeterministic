...
export function drawRarityAura(ctx: CanvasRenderingContext2D, rarity: RarityLevel): void {
  switch (rarity) {
    ...
    case "uncommon": {
      ...
      outerGlow(ctx, "rgba(0,200,60,0.22)", 1, 64);
      glowRim(ctx, "#00c83c", 0.6, 12, 3);
      particles(ctx, "#44ff88", 10, 48, 62, 2.2, 0.8);
      break;
    }
    case "rare": {
      // Blue pulsing rim + corner rays + blue particles
      outerGlow(ctx, "rgba(0,112,221,0.28)", 1, 64);
      glowRim(ctx, "#0070dd", 0.7, 16, 4);
      rays(ctx, "#4ea8ff", 12, 12, 0.6);
      particles(ctx, "#80c8ff", 16, 46, 62, 2.5, 0.85);
      ...
      for (const [cx2, cy2] of [[6, 6], [122, 6], [6, 122], [122, 122]]) {
        ...
        ctx.fillStyle = "#80c8ff"; ctx.shadowColor = "#0070dd"; ctx.shadowBlur = 10; ctx.fill();
      }
      ctx.restore();
      break;
    }
    case "epic": {
      ...
      outerGlow(ctx, "rgba(163,53,238,0.35)", 1, 64);
      glowRim(ctx, "#a335ee", 0.8, 20, 5);
      particles(ctx, "#cc88ff", 14, 46, 64, 3, 0.9);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        g.addColorStop(0, "#ffffff"); g.addColorStop(0.4, "#b060ff"); g.addColorStop(1, "transparent");
        ctx.fillStyle = g; ctx.shadowColor = "#9030d0"; ctx.shadowBlur = 12; ctx.fill();
      }
      ctx.restore();
      ...
      ctx.globalCompositeOperation = "screen";
      ctx.strokeStyle = "#a335ee"; ctx.lineWidth = 1.5; ctx.shadowColor = "#a335ee"; ctx.shadowBlur = 8;
    ...
    case "legendary": {
      ...
      outerGlow(ctx, "rgba(255,128,0,0.4)", 1, 64);
      glowRim(ctx, "#ff8000", 0.9, 24, 6);
      rays(ctx, "#ffd060", 20, 16, 0.75);
      particles(ctx, "#ffe080", 20, 44, 64, 2.8, 0.95);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, -62); ctx.lineTo(6, -15); ctx.lineTo(0, 0); ctx.lineTo(-6, -15);
      ...
      for (const [ox, oy] of [[8, 8], [120, 8], [8, 120], [120, 120]]) {
        ...
        ctx.fillStyle = "#fff5b0"; ctx.shadowColor = "#ff8000"; ctx.shadowBlur = 12;
    ...
    case "mythic": {
      ...
      outerGlow(ctx, "rgba(255,100,200,0.45)", 1, 64);
      glowRim(ctx, "#ff50d0", 0.95, 30, 8);
      rays(ctx, "#ff80e8", 32, 18, 0.65);
      particles(ctx, "#ffaaff", 24, 44, 64, 3.5, 1.0);
      // Orbiting rune ring
      ctx.save();
      ctx.strokeStyle = "#cc40b0"; ctx.lineWidth = 1.5; ctx.globalAlpha = 0.6;
      ctx.shadowColor = "#ff40cc"; ctx.shadowBlur = 12;
      ctx.globalCompositeOperation = "screen";
      ...
      for (let i = 0; i < 8; i++) {
        ...
        g.addColorStop(0, "#ffffff"); g.addColorStop(0.5, "#ffaaff"); g.addColorStop(1, "transparent");
        ctx.fillStyle = g; ctx.shadowColor = "#ff40cc"; ctx.shadowBlur = 14; ctx.fill();
      }
      ctx.restore();
      // Cosmic shimmer center
      const cg = ctx.createRadialGradient(CX, CY, 0, CX, CY, 42);
      cg.addColorStop(0, "rgba(255,200,255,0.2)");
...
export function drawElementEffect(ctx: CanvasRenderingContext2D, element: ElementType): void {
  ...
  switch (element) {
    case "fire": {
      ...
      ctx.globalCompositeOperation = "screen";
      const fg = ctx.createRadialGradient(CX, SIZE - 10, 15, CX, SIZE - 5, 60);
      ...
      const flames: [number, number, number][] = [
        [CX - 20, 95, 28], [CX - 8, 70, 36], [CX + 6, 85, 30], [CX + 18, 92, 25],
        [CX - 30, 100, 20], [CX + 30, 98, 18], [CX, 60, 42]
      ];
      for (const [fx, fy, fh] of flames) {
        const lg = ctx.createLinearGradient(fx, fy, fx, fy - fh);
        lg.addColorStop(0, "rgba(255,50,0,1.0)");
        lg.addColorStop(0.3, "rgba(255,140,0,0.8)");
        lg.addColorStop(0.7, "rgba(255,220,50,0.4)");
        lg.addColorStop(1, "transparent");
        ctx.beginPath();
        ctx.moveTo(fx - 6, fy);
        ctx.bezierCurveTo(fx - 10, fy - fh * 0.4, fx + 8, fy - fh * 0.7, fx, fy - fh);
        ctx.bezierCurveTo(fx - 8, fy - fh * 0.7, fx + 10, fy - fh * 0.3, fx + 6, fy);
        ctx.closePath();
        ctx.fillStyle = lg; ctx.shadowColor = "#ff4400"; ctx.shadowBlur = 12; ctx.fill();
      }

      const sparks: [number, number, number][] = [
        [CX - 25, 60, 2.5], [CX + 22, 55, 2.0], [CX - 12, 45, 2.8], [CX + 10, 40, 2.2],
        [CX - 35, 75, 2.0], [CX + 32, 65, 2.5], [CX + 5, 30, 1.8], [CX - 5, 20, 1.5]
      ];
      ctx.fillStyle = "#ffffff"; ctx.shadowColor = "#ffcc00"; ctx.shadowBlur = 10;
    ...
    case "ice": {
      ...
      ctx.globalCompositeOperation = "screen";
      const ig = ctx.createRadialGradient(CX, CY, 20, CX, CY, 80);
      ig.addColorStop(0, "rgba(255,255,255,0.1)");
      ig.addColorStop(0.6, "rgba(100,220,255,0.25)");
      ig.addColorStop(1, "transparent");
      ctx.fillStyle = ig; ctx.fillRect(0, 0, SIZE, SIZE);

      const shards: [number, number, number, number][] = [
        [14, 20, -0.4, 18], [12, 55, -0.6, 14], [16, 85, -0.3, 16],
        [114, 18, 0.4, 17], [116, 60, 0.5, 15], [112, 90, 0.35, 13],
        [30, 6, -0.8, 14], [64, 8, 0, 12], [100, 9, 0.7, 15],
        [28, 118, -0.5, 13], [68, 120, 0, 12], [96, 116, 0.6, 14],
      ];
      for (const [sx, sy, angle, sh] of shards) {
        ...
        sg.addColorStop(0.4, "rgba(200,240,255,1.0)");
        sg.addColorStop(1, "transparent");
        ctx.beginPath();
        ctx.moveTo(-6, 0); ctx.lineTo(0, -sh); ctx.lineTo(6, 0); ctx.closePath();
        ctx.fillStyle = sg; ctx.shadowColor = "#ffffff"; ctx.shadowBlur = 10; ctx.fill();
        ctx.restore();
      }

      ctx.strokeStyle = "rgba(220,250,255,0.6)"; ctx.lineWidth = 1.5;
    ...
    case "electro": {
      ...
      ctx.globalCompositeOperation = "screen";
      const eg = ctx.createRadialGradient(CX, CY, 10, CX, CY, 75);
      eg.addColorStop(0, "rgba(255,255,255,0.2)");
      eg.addColorStop(0.4, "rgba(255,240,0,0.2)");
      eg.addColorStop(0.8, "rgba(160,80,255,0.15)");
      ...
      const bolts: [number, number, number, number, number[]][] = [
        ...
        [5, 64, 123, 64, [25, 55, 50, 75, 75, 50, 100, 68]]
      ];

      ctx.lineWidth = 2.2; ctx.lineCap = "round";
      for (const [x1, y1, x2, y2, pts] of bolts) {
        ctx.strokeStyle = "#fff500"; ctx.shadowColor = "#ffee00"; ctx.shadowBlur = 12;
        ctx.beginPath(); ctx.moveTo(x1, y1);
        for (let i = 0; i < pts.length - 1; i += 2) { ctx.lineTo(pts[i], pts[i + 1]); }
        ctx.lineTo(x2, y2); ctx.stroke();

        ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 1.0; ctx.shadowBlur = 6;
        ctx.beginPath(); ctx.moveTo(x1, y1);
        for (let i = 0; i < pts.length - 1; i += 2) { ctx.lineTo(pts[i], pts[i + 1]); }
        ctx.lineTo(x2, y2); ctx.stroke();
        ctx.lineWidth = 2.2;
      ...
      ctx.strokeStyle = "#d0a0ff"; ctx.lineWidth = 1.5; ctx.shadowColor = "#a335ee"; ctx.shadowBlur = 8;
